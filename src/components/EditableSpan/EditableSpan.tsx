...
import { changeTaskTitleTC } from "store/slices/tasks.slice"
import { changeTodoTitleTC } from "store/slices/todoLists.slice"
...
const EditableSpan: FC<EditableSpanPropsType> = React.memo((props) => {
    ...
    const inputOfNewTitleOnChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const inputOnKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
        if (event.key === "Enter") {
            if (props.spanFor === "task") {
                dispatch(changeTaskTitleTC(props.todoListId, props.elementId, inputOfNewTaskTitle))
                setIsEditMode(false)
            } else {
                dispatch(changeTodoTitleTC(props.elementId, inputOfNewTaskTitle))
                setIsEditMode(false)
            }
        }
    }
